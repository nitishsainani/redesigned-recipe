...
for i in range(total_recipes):
    try:
        ...
        for nutrient in recipe.get('nutrition', []):
            quantity = None if not recipe['nutrition'][nutrient].strip().strip('-') else float(recipe['nutrition'][nutrient])
        ...
        print(str(e))
        if str(e) == 'UNIQUE constraint failed: recipes_recipe.title':
            fails.append(recipes[i])
        continue

with open('fails2.json', 'w+') as out:
